...
## Definition of Done
A feature is NOT complete until its data path is real end-to-end.
"Looks like it works" is NOT done. "Data flows through real I/O" IS done.

## Build Order: Backend First
1. Create real database tables FIRST (SQLite WAL mode unless spec says otherwise).
2. Implement real API endpoints with real CRUD against the database SECOND.
3. Build frontend LAST, connecting to the real API.
Never build UI before the backend it depends on exists.

## Absolute Prohibitions
- NO MOCKS. NO STUBS. NO HARDCODED ARRAYS. NO FAKE DATA.
- NO returning static JSON from API endpoints.
- NO "placeholder" implementations that skip database operations.
- NO UI that displays fake data or simulates functionality.
- If a feature's backend is not ready, show "Not implemented" in the UI explicitly. Do NOT fake it.

## Implementation Requirements
1. Every API endpoint in the spec MUST have real database queries.
2. All forms MUST save to and read from the database. Verify by inserting test data and confirming it persists.
3. Server MUST run on port 8000. Install as a systemd service.
4. Serve frontend from the same server on port 8000.
5. Include input validation, error handling, and logging.
6. If the spec mentions auth, implement real session-based auth, not a fake login.

## Verification (MANDATORY before marking complete)
1. Open the browser and test every page.
2. Submit a form, then reload the page — data must persist.
3. Check the database directly: sqlite3 <db> "SELECT * FROM <table>" must show real rows.
4. If any feature returns mock data, fix it before proceeding.
5. Commit all code with git.
