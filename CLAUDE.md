...
- **完成 = データ経路が実物であること。** UI が表示されるだけでは完成ではない。
- **バックエンド未完成なら、先にバックエンドの最小実装（仮でも本物の I/O）を作る。**
- **UI だけ先にモックで作って完成に見せるのは禁止。**
- 未実装部分は UI 上で「Not implemented」と明示する（ユーザーに嘘をつかない）。

### 実装順序: バックエンドファースト

1. データベーステーブルを先に作る
2. API エンドポイントを実装する（実際の DB 操作付き）
3. フロントエンドを最後に作り、本物の API に接続する

### 検証手順（完了前に必ず実行）

1. ブラウザで全ページをテストする
2. フォームを送信し、ページをリロードしてデータが消えないことを確認
3. DB を直接確認: `sqlite3 <db> "SELECT * FROM <table>"` で実データがあること
4. モックデータを返している箇所があれば修正してから次に進む

### 完了基準 — 「見た目が動く」を完成扱いにしない

- データ経路が実物であることを完了条件にする
- もしバックエンド未完成なら、先にバックエンド側の最小実装（仮でも本物の I/O）を作る
- 「UI だけ先にモックで作って完成に見せる」は禁止
- 未実装部分は UI 上で明示的に "Not implemented" と表示する（ユーザーに嘘をつかない）
- AI エージェント（Shelly 等）が PRD を元に実装する際も、このルールを遵守する
