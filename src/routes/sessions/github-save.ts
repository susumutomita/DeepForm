...
export function generateAgentMd(theme: string, prd: Record<string, unknown>, spec: Record<string, unknown>): string {
  const specRaw = typeof spec.spec === "object" && spec.spec !== null ? (spec.spec as Record<string, unknown>).raw : "";
  const specText = typeof specRaw === "string" ? specRaw : JSON.stringify(spec, null, 2);
...
- **No mock data**: All data must come from real DB/API connections
- **Backend-first**: Implement API endpoints before building UI
- **Show "Not implemented"**: Never fake functionality with stubs
...
export function generatePlanMd(theme: string, prd: Record<string, unknown>, spec: Record<string, unknown>): string {
  const specObj = typeof spec.spec === "object" && spec.spec !== null ? (spec.spec as Record<string, unknown>) : spec;
  const specRaw = typeof specObj.raw === "string" ? specObj.raw : "";
...
githubSaveRoutes.post("/sessions/:id/github-save", async (c) => {
  try {
    ...
    // spec の raw テキストを抽出
    const specObj = typeof specData.spec === "object" && specData.spec !== null ? specData.spec : specData;
    const specRaw = typeof specObj.raw === "string" ? specObj.raw : JSON.stringify(specData, null, 2);

    // 6. ファイルを並行生成
    const [readme, agentMd, planMd] = await Promise.all([
      generateReadme(session.theme, prdMarkdown, specRaw),
      Promise.resolve(generateAgentMd(session.theme, prd, specData)),
      Promise.resolve(generatePlanMd(session.theme, prd, specData)),
    ]);
