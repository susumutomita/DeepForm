...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!factsRow) {
        res.status(400).json({ error: 'No facts found. Run analyze first.' });
        return;
      }
...
app.post('/api/sessions/:id/prd', async (req: Request, res: Response) => {
  try {
    ...
    if (!factsRow || !hypRow) {
      res.status(400).json({
        error:
          'Facts and hypotheses required. Run analyze and hypotheses first.',
      });
      return;
    }
...
app.post('/api/sessions/:id/spec', async (req: Request, res: Response) => {
  try {
    ...
    if (!prdRow) {
      res
        .status(400)
        .json({ error: 'PRD required. Run prd generation first.' });
      return;
    }
